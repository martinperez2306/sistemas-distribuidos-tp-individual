...
class TrendingFilterCaller(RoutingCaller):
    ...
    def filter_by_trending(self, message: Message):
        filter_by_trending_message = Message(MIDDLEWARE_MESSAGE_ID, message.request_id, message.source_id, message.operation_id, TRENDING_FILTER_GROUP_ID, message.body)
        ...
        elif LOAD_TOTAL_COUNTRIES == message.operation_id:
            self.__broadcast(filter_by_trending_message)
