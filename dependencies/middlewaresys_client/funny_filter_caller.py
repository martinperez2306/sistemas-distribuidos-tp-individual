...
class FunnyFilterCaller(RoutingCaller):
    ...
    def filter_by_funny_tag(self, message: Message):
        filter_by_tag_message = Message(SERVICE_MESSAGE_ID, message.request_id, message.source_id, message.operation_id, FUNNY_FILTER_GROUP_ID, message.body)
        if START_PROCESS_OP_ID == message.operation_id:
            ...
            if not self.connection or not self.connection.is_open:
                self.connect()
            video = Video.from_json(filter_by_tag_message.body)
            video_id = video.id
