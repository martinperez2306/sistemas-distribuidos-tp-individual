...
SERVICE_ID="SERV_ID"
cat $INGESTION_TEMPLATE | sed -r "s/$INGESTION_SERVICE/$INGESTION_NAME/g" | sed -r "s/$CONTAINER_NAME/$INGESTION_NAME/g" | sed -r "s/$SERVICE_ID/$INGESTION_NAME/g" >> $OUTPUT
echo -e "\n" >> $OUTPUT
...
MAX_NAME="max_1"
SERVICE_ID="SERV_ID"
cat $MAX_TEMPLATE | sed -r "s/$MAX/$MAX_NAME/g" | sed -r "s/$CONTAINER_NAME/$MAX_NAME/g" | sed -r "s/$SERVICE_ID/$MAX_NAME/g" >> $OUTPUT
echo -e "\n" >> $OUTPUT
...
SERVICE_ID="SERV_ID"
cat $REPORTING_TEMPLATE | sed -r "s/$REPORTING_SERVICE/$REPORTING_NAME/g" | sed -r "s/$CONTAINER_NAME/$REPORTING_NAME/g" | sed -r "s/$SERVICE_ID/$REPORTING_NAME/g" >> $OUTPUT
echo -e "\n" >> $OUTPUT
